...
export const useElementInspector = () => {
  const { state, selectElement, updateStyle, closeInspector } = useBrandBook();
  ...
  const handleStyleChange = useCallback(
    (elementId: string, property: string, value: any) => {
      try {
        // Validate inputs
        if (!elementId || !property) {
          console.warn('⚠️ Invalid style change:', { elementId, property, value });
          return;
        }
  ...
  const handleBatchStyleChange = useCallback(
    (elementId: string, styles: Record<string, any>) => {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('✨ Batch styles updated:', { elementId, count: Object.keys(styles).length });
        }
      } catch (error) {
   ...
   */
  const hasCustomStyles = useCallback(
    (elementId: string): boolean => {
      const style = state.customStyles[elementId];
      return !!(
        style &&
        (style.classes || style.content || Object.keys(style.style || {}).length > 0)
      );
    },
    [state.customStyles]
  );
  ...
  const resetElementStyles = useCallback(
    (elementId: string) => {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('🔄 Styles reset:', { elementId });
        }
      } catch (error) {
   ...
   */
  const getComputedStyleString = useCallback(
    (elementId: string): string => {
      const customStyle = state.customStyles[elementId];
      if (!customStyle?.style) return '';

      return Object.entries(customStyle.style)
