...
export const ColorSwatch: React.FC<ColorSwatchProps> = memo(({
  ...
  const elementId = useMemo(() => `color-${name.toLowerCase().replace(/\s+/g, '-')}`, [name]);
  ...
  const isSelected = useMemo(() => isElementSelected(elementId), [isElementSelected, elementId]);

  // Check if this color was just copied
  const isCopied = useMemo(() => state.copiedColor === name, [state.copiedColor, name]);
   ...
   */
  const glowShadow = useMemo(() => `0 0 20px ${value}80, 0 0 40px ${value}40`, [value]);
  return <motion.div {...ANIMATION_VARIANTS.scaleIn} whileHover={{
    ...
    damping: 20
      ...
      `} style={customStyles.style} onClick={handleClick} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} role="button" tabIndex={0} aria-label={`${name} color: ${value}. Click to copy, Shift+Click to edit`} aria-pressed={isSelected} onKeyDown={handleKeyDown}>
      {/* Edit Icon Indicator */}
      <AnimatePresence>
        ...
        duration: 0.2
            ...
            <Edit3 className="w-3 h-3 text-[#BEF264] drop-shadow-lg" aria-hidden="true" />
          </motion.div>}
      </AnimatePresence>
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center justify-between mb-1">
          ...
          <AnimatePresence mode="wait">
            ...
            type: 'spring'
                ...
                <Check className="w-4 h-4 text-[#BEF264]" aria-label="Copied to clipboard" />
            ...
            duration: 0.2
                ...
                <Copy className="w-4 h-4 text-zinc-500" aria-hidden="true" />
              </motion.div>}
          </AnimatePresence>
        </div>
        ...
        {description && <motion.p className="text-xs text-zinc-500 mt-1 line-clamp-2" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} transition={{
      ...
      <AnimatePresence>
        ...
        duration: 0.2
      }} className="absolute bottom-2 left-2 text-[8px] text-zinc-600 font-medium bg-zinc-800/50 px-2 py-1 rounded backdrop-blur-sm" aria-hidden="true">
            ⇧ Shift+Click to edit
          </motion.div>}
      </AnimatePresence>

      {/* Selected Indicator (Visual Ring) */}
      {isSelected && <motion.div className="absolute inset-0 pointer-events-none rounded-xl" initial={{
      opacity: 0
    }} animate={{
      opacity: 1
    }} style={{
