...
const SidebarMenuItem: React.FC<SidebarMenuItemProps> = memo(({
  ...
  if (href) {
    return <Link to={href} onClick={onClick} aria-current={isActive ? 'page' : undefined} className={baseClasses}>
          {content}
        </Link>;
  }
  return <button onClick={onClick} type="button" aria-current={isActive ? 'page' : undefined} className={baseClasses}>
        {content}
      </button>;
});
